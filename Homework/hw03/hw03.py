...
def replace_leaf(t, find_value, replace_value):
    ...
    else: return tree(label(t), [replace_leaf(b, find_value, replace_value) for b in branches(t)])
...
def has_path(t, word):
    ...
    if word == '': return True
        ...
        if len(word) == 1: return True
            ...
            for b in branches(t):
                if has_path(b, word[1:]): return True
    return False
...
def quadratic(x, a, b, c):
    ...
    if a == 0: return interval(min(f(l), f(r)), max(f(l), f(r)))
    m = - b / (2 * a)
    if r <= m or l >= m: return interval(min(f(l), f(r)), max(f(l), f(r)))
    else: 
        if a > 0: return interval(c - (b ** 2) / (4 * a), max(f(l), f(r)))
        else: return interval(min(f(l), f(r)), c - (b ** 2) / (4 * a))
