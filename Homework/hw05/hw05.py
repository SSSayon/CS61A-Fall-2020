class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.stock -= 1
            _change = self.balance - self.price
            self.balance = 0
            return 'Here is your {0} and ${1} change.'.format(self.product, _change)
...
def store_digits(n):
    ...
    def get_list(lst, n):
        if n == 0:
            ...
            return get_list(Link(n % 10, lst), n // 10)
    return get_list(Link.empty, n)
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        return is_bst(t.branches[0]) and is_bst(t.branches[1])\
               and bst_max(t.branches[0]) <= t.label < bst_min(t.branches[1])
    else:
        return False
