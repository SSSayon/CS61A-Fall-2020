...
def pig_pass(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4b
    if (player_score + 3 > opponent_score)&(player_score < opponent_score):
        return True
    return False
    # END PROBLEM 4b
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max, pt = 0, 0
    for i in range(10):
        tmp = make_averaged(roll_dice, trials_count)(i+1, dice)
        if tmp > max:
            max, pt = tmp, i+1
    return pt
    # END PROBLEM 9
...
def final_strategy(score, opponent_score, winratecat=0):
    """Write a brief description of your final strategy.

    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    # strategy1: as long as I can 100% get an extra turn, do it.
    if extra_turn(score + free_bacon(opponent_score), opponent_score):
        return 0
    # strategy2: if my score is close to 100, try to win at once
    score_by_0 =free_bacon(opponent_score)
    if (score + score_by_0 >= GOAL_SCORE):
        return 0
    winrate = [ [2, 2, 2, -1, -1, -1, -1], [2, 2, 2, 3, 3, 3, -1], [2, 2, 2, 3, 3, 3, 3] ]
      # 60%, 50%, 45%exi
    needed_score = GOAL_SCORE - score
    if (needed_score >=4)&(needed_score <= 10):
        step = winrate[winratecat][needed_score - 4]
        if (step > 0): return step
    # strategy3: get the most expected score
    expected_scores = [3.48778, 5.85595, 7.36894, 8.19761, 8.63681, 
                       8.65712, 8.502, 8.25604, 7.81371, 7.25094]
    if score_by_0 > max(expected_scores): return 0
    else:
        max_expected_score = max(expected_scores)
        index = expected_scores.index(max_expected_score)
        return index + 1
    # unrealized strategy: when I precede greatly, choose to roll less times to avoid getting 1 score.
    # when I'm doomed to loss, tend to roll more dices to take a chance.
    # END PROBLEM 12

def test(total=100000):
    for i in range(4, 11):
        print('when needs', i, 'scores: \n')
        for j in range(1, 10):
            cnt = 0
            for k in range(total):
                if roll_dice(j) >= i: cnt += 1
            p = cnt / total
            print('    ', j, p)
        print('\n')

def test2(total=100000):
    for i in range(1, 11):
        expected_score = make_averaged(roll_dice, total)(i)
        print(expected_score, end=', ')
##########################
