...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant) and not isinstance(place.ant, ContainerAnt):
                self.contain_ant(place.ant)
                place.ant = self
            elif place.ant.can_contain(self) and not isinstance(self, ContainerAnt):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dist = 0
        while cur_place is not beehive and cur_place.entrance is not None:
            if len(cur_place.bees) > 0\
                and self.min_range <= dist <= self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting == 0:
            ...
            self.digesting = max(0, self.digesting - 1)
        # END Problem 6
...
# BEGIN Problem 7
class WallAnt(Ant):
    """WallAnt provides protection and nothing else."""
    name = 'Wall'
    food_cost = 4
    implemented = True   # Change to True to view in the GUI
        ...
        if not self.is_first_queen:
            ...
            while cur_place is not None:
                if cur_place.ant is not None:
                    if cur_place.ant not in self.buffed_ants:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        distance = 0
        while place != beehive:
            if place.ant is not None and place.ant != self:
