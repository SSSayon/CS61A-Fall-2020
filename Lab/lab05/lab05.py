...
def closer_city(lat, lon, city_a, city_b):
    ...
    if distance(tmp, city_a) < distance(tmp, city_b): return get_name(city_a)
...
def sprout_leaves(t, leaves):
    ...
    """
    "*** YOUR CODE HERE ***"
    if is_leaf(t): return tree(label(t), [tree(i) for i in leaves])
...
def add_trees(t1, t2):
    ...
    elif t1 is None: return tree(label(t2), [add_trees(None, branch) for branch in branches(t2)])
    elif t2 is None: return tree(label(t1), [add_trees(None, branch) for branch in branches(t1)])
    else:
        _label = label(t1) + label(t2)
        _branches = []
        for branch1, branch2 in zip_longest(branches(t1), branches(t2)):
            _branches.append(add_trees(branch1, branch2))
        return tree(_label, _branches)
